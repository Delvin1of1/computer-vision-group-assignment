...
% Modified to iterate through folders of a Dataset

% Pure HOMO-Feature cross-modal image matching demo.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% Modified by Group 1, CPEG-585 Computer Vision Spring 2026
% Peter Koropey, Delvin Addo, Davide Luppi, Yaser Al-Hameedi

% This file performs the confirmation test described in the report.
% It needs to be put in the main "HOMO_image_matching_demo" folder to Run.
% IE, the folder with the "A_HOMO_demo.m" file

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

close all; clc; clear;
addpath('functions','func_Math','func_Geo','func_HOMO');
save_path = '.\HOMO_confirmation_test\';
...
Is_flag  = 1;  % Do you want the visualization of registration resultsen
...
for k = 1:length(files1)
    ...
    tic,[indexPairs,matchmetric] = matchFeatures(cor1, cor2, Metric="SAD", Unique=true);
    cor1_r = cor1/resample1; cor2_r = cor2/resample2;
    %matchedPoints1 = cor1(indexPairs(:,1),:);
    %matchedPoints2 = cor2(indexPairs(:,2),:);
    %disp(numel(indexPairs));
    threshold=3;
    passCount = sum(matchmetric <= threshold);
    %disp(passCount);

    %Print to File
    outputStringTab = strcat(file1(end-9:end-4),'\t\t',string(numel(indexPairs)),'\t\t',string(passCount),'\n');
    outputStringComma = strcat(file1(end-9:end-4),',',string(numel(indexPairs)),',',string(passCount),'\n');
    fprintf(outputStringTab);
    fprintf(fileID,outputStringComma);
	%First column is Image ID, 2nd Column is No. Matches detected, 3rd Column is No. matches accepted
	%IE, No. Matches with <3 pixels error
...
end %end of outer for loop for iterating through folder
