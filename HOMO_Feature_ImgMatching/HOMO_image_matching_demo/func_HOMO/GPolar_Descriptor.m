...
% Theta area divide
Theta0 = atan2(YY,XX) + pi;  % 取值范围：[0,2pi]
if ~rot_flag
    Theta = mod(floor(Theta0*NBA/pi/2),NBA)+1;  % 取值范围：[1,NBA]
end
...
for k = 1:size(kps,1)
    ...
    if rot_flag
        ...
        angle_bin(W+y1-y+1:W+y2-y+1, W+x1-x+1:W+x2-x+1) = ...
            mod(floor((MOM_o(y1:y2, x1:x2)-orient) * NBO/pi), NBO) + 1;  % 取值范围：[1,NBO] & 0
        Theta = mod(floor((Theta0-orient)*NBA/pi/2), NBA) + 1;  % 取值范围：[1,NBA]
%         sin_p = sin(orient); cos_p = cos(orient);
